			...
			for loan in loans: # iterate through all loans
				...
				if len(dataColumns) > 0: # if 'loan' is actually a loan
					...
					if dataColumns[7].text == "Yes":
						...
						loanTuple['status'] = 2
					loanTuple['dateLent'] = datetime.datetime.strptime(dataColumns[8].text, '%Y-%m-%d %H:%M:%S').date()
					try:
						loanTuple['datePaid'] = datetime.datetime.strptime(dataColumns[9].text, '%Y-%m-%d %H:%M:%S').date()
